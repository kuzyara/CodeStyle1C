...
Подозрительный код (todo: пример найти получше, подозрительного тут ничего не вижу)
```bsl
// Плохо:
...
```bsl
// Хорошо:
Если ОткрыватьОбъектПриИдентификаторе(ИдентификаторОбъекта) Тогда // <- спрятали все проверки в функцию ОткрыватьОбъектПриИдентификаторе()
