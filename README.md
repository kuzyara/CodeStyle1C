...
__Исключения:__ использование версий ```EnterpriseData_1_20```, ```ГрафикЭтаповПроизводства2_2```, аббревиатур ```Лого1С_БРО```, ```БазоваяВерсияУТ_ИП```, языка ```Драйвер1СДисплейПокупателя_en```, расширений ```ПФ_MXL_КорректировочныйСчетФактура1096```, префиксов ```БСП_Анкета```, ```Расш1_ПриСозданииНаСервереВместо```
