# Соглашения по стилю кода 1С
 ...
 Тренажер код-ревью: https://codereview1c.ru/
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
    ...
    КонецЕсли;
```
В данном случае используется вызов исключения вместо установки параметра Отказ. При [коридорном тестировании](https://www.google.com/search?q=коридорное+тестирование) неосведомленный человек будет поставлен в тупик "некорректным" статусом. Так же в сообщение требуется добавить: а) какой сейчас статус, б) и какой нужен.
