...
При обмене данными через шину данных, например RabbitMQ, встречается задача сериализации ссылок методом `ЗаписатьJSON`:
```bsl
...
Справочники.ИсходящиеСообщения.Опубликовать("goods.batch.negative.balance", СтрокаJSON); // отправка
```
Для передачи ссылки можно придумать множество способов, далее описано какие встречались мне на практике.
